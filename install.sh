...
awk -v iface="$network_interface" '/^config device$/ { getline; if (index($0, "option name \047" iface "\047")) { while (getline > 0 && $1 != "config") { if ($1 == "option" && $2 == "ports") { for (i=3; i<=NF; i++) { gsub(/[\047\042]/, "", $i); if ($i != "") print $i } } } } }' /etc/config/network > "$available_ports_file"
